...
public class FineGrainedDrivingControl extends CommandBase {
  private final DriveTrain m_drivetrain;
  private final CommandXboxController m_driver_controller;

  /** Creates a new FineGrainedDrivingControl. */
  public FineGrainedDrivingControl(DriveTrain dt, CommandXboxController xbox) {
    m_drivetrain = dt;
  ...
  public void execute() {
    ...
    m_drivetrain.fine_grained_drive(forward, turn);
  }
  ...
  public boolean isFinished() {
    // This command will be bound to a held button, so returning false means that it will be
    // run until it's not scheduled anymore.
    return false;
  }
}
